...
def fork_repo(attendee_state, config, facilitator_state, org_name):
    facilitator_handles = [facilitator["handle"] for facilitator in facilitator_state]
    facilitator_handles.append(config["billing-admin"])

    def retry_fork(repo, org_name, retries=3, delay=5):
        """Retries forking with exponential backoff."""
        for attempt in range(retries):
            try:
                admin_client.repo.fork(repo, org_name)
                # Make forked repos private, except for .github
                if repo.split("/")[1] != ".github":
                    forked_repo = f"{org_name}/{repo.split('/')[1]}"
                    time.sleep(15)  # Avoid race condition before setting visibility
                ...
                if attempt < retries - 1:
                    ...
                    print(f"Failed to fork {repo} after {retries} attempts.")
                    return False  # Failure

    for attendee in attendee_state:
        for repo in config["repos-to-fork"]:
            success = retry_fork(repo, org_name)
            if not success:
                attendee["fork_errors"].append(repo)
            time.sleep(5)  # Delay between forks
